...
<p align="center">
  <b><i><font size="5">Chatea con <a href="#-integracion-con-openclaw">OpenClaw</a>: "Investiga X" 鈫?hecho.</font></i></b>
</p>
...
<p align="center">
  <a href="showcase/SHOWCASE.md">馃弳 Galeria de articulos</a> 路 <a href="integration-guide.md">馃摉 Guia de integracion</a> 路 <a href="https://discord.gg/u4ksqW5P">馃挰 Comunidad Discord</a>
</p>
...
<td valign="middle">
<b>馃弳 Galeria de articulos generados</b><br><br>
<b>8 articulos en 8 dominios</b> 鈥?matematicas, estadistica, biologia, computacion, NLP, RL, vision, robustez 鈥?generados de forma completamente autonoma sin intervencion humana.<br><br>
...
> **馃И Buscamos testers!** Prueba el pipeline con tu propia idea de investigacion 鈥?de cualquier campo 鈥?y [cuentanos que piensas](TESTER_GUIDE.md). Tu feedback da forma directamente a la proxima version. **[鈫?Testing Guide](TESTER_GUIDE.md)** | **[鈫?涓枃娴嬭瘯鎸囧崡](TESTER_GUIDE_CN.md)** | **[鈫?鏃ユ湰瑾炪儐銈广儓銈偆銉塢(TESTER_GUIDE_JA.md)**
...
Proporciona un tema de investigacion 鈥?recibe un articulo academico completo con literatura real de OpenAlex, Semantic Scholar y arXiv, experimentos en sandbox adaptados al hardware (deteccion automatica GPU/MPS/CPU), analisis estadistico, revision multi-agentes, y LaTeX listo para conferencia orientado a NeurIPS/ICML/ICLR. Sin supervision. Sin copiar y pegar. Sin referencias alucinadas.

<table>
<tr><td>馃搫</td><td><code>paper_draft.md</code></td><td>Articulo academico completo (Introduccion, Trabajo relacionado, Metodo, Experimentos, Resultados, Conclusion)</td></tr>
<tr><td>馃搻</td><td><code>paper.tex</code></td><td>LaTeX listo para conferencia (plantillas NeurIPS / ICLR / ICML)</td></tr>
<tr><td>馃摎</td><td><code>references.bib</code></td><td>Referencias BibTeX reales de OpenAlex, Semantic Scholar y arXiv 鈥?auto-depuradas para coincidir con las citas en linea</td></tr>
<tr><td>馃攳</td><td><code>verification_report.json</code></td><td>Verificacion de integridad + relevancia de citas en 4 capas (arXiv, CrossRef, DataCite, LLM)</td></tr>
<tr><td>馃И</td><td><code>experiment runs/</code></td><td>Codigo generado + resultados en sandbox + metricas JSON estructuradas</td></tr>
<tr><td>馃搳</td><td><code>charts/</code></td><td>Graficos de comparacion de condiciones auto-generados con barras de error e intervalos de confianza</td></tr>
<tr><td>馃摑</td><td><code>reviews.md</code></td><td>Revision por pares multi-agente con verificacion de consistencia metodologia-evidencia</td></tr>
<tr><td>馃К</td><td><code>evolution/</code></td><td>Lecciones de auto-aprendizaje extraidas de cada ejecucion</td></tr>
<tr><td>馃摝</td><td><code>deliverables/</code></td><td>Todos los entregables finales en una sola carpeta 鈥?listos para compilar en Overleaf</td></tr>
</table>
...
馃實 **Ejecutalo en cualquier lugar.** AutoResearchClaw no esta atado a una sola plataforma. Usalo de forma independiente por CLI, conectalo a [OpenClaw](https://github.com/openclaw/openclaw), o integralo con cualquier agente compatible con ACP 鈥?馃 Claude Code, 馃捇 Codex CLI, 馃悪 Copilot CLI, 鈾?Gemini CLI, 馃寵 Kimi CLI, y mas. Gracias al puente de mensajeria de OpenClaw, puedes iniciar una investigacion completa desde 馃挰 Discord, 鉁堬笍 Telegram, 馃惁 Lark (椋炰功), 馃挌 WeChat, o cualquier plataforma que tu equipo ya utilice. Un tema de entrada, un paper de salida 鈥?sin importar donde lo escribas.
...
Salida 鈫?`artifacts/rc-YYYYMMDD-HHMMSS-<hash>/deliverables/` 鈥?LaTeX listo para compilar, BibTeX, codigo experimental, graficos.

<details>
<summary>馃摑 Configuracion minima requerida</summary>
...
## 馃 Que lo hace diferente

| Capacidad | Como funciona |
|-----------|--------------|
| **馃攧 Bucle PIVOT / REFINE** | La etapa 15 decide de forma autonoma: PROCEED, REFINE (ajustar parametros) o PIVOT (nueva direccion). Artefactos auto-versionados. |
| **馃 Debate multi-agente** | La generacion de hipotesis, el analisis de resultados y la revision por pares utilizan cada uno debate estructurado multi-perspectiva. |
| **馃К Auto-aprendizaje** | Lecciones extraidas por ejecucion (justificacion de decisiones, advertencias de ejecucion, anomalias de metricas) con decaimiento temporal de 30 dias. Las ejecuciones futuras aprenden de errores pasados. |
| **馃摎 Base de conocimiento** | Cada ejecucion construye una KB estructurada en 6 categorias (decisiones, experimentos, hallazgos, literatura, preguntas, revisiones). |
| **馃洝锔?Vigilante Sentinel** | Monitor de calidad en segundo plano: deteccion NaN/Inf, consistencia articulo-evidencia, puntuacion de relevancia de citas, guardia anti-fabricacion. |

---

## 馃 Integracion con OpenClaw

<table>
<tr>

**AutoResearchClaw es un servicio compatible con [OpenClaw](https://github.com/openclaw/openclaw).** Instalalo en OpenClaw y lanza investigacion autonoma con un solo mensaje 鈥?o usalo de forma independiente via CLI, Claude Code o cualquier asistente de programacion con IA.

</tr>
</table>

### 馃殌 Uso con OpenClaw (Recomendado)

Si ya usas [OpenClaw](https://github.com/openclaw/openclaw) como tu asistente de IA:

```
1锔忊儯  Comparte la URL del repositorio de GitHub con OpenClaw
2锔忊儯  OpenClaw lee automaticamente RESEARCHCLAW_AGENTS.md 鈫?comprende el pipeline
3锔忊儯  Di: "Research [tu tema]"
4锔忊儯  Listo 鈥?OpenClaw clona, instala, configura, ejecuta y devuelve los resultados
```
...
<details>
<summary>馃挕 Que sucede internamente</summary>

1. OpenClaw lee `RESEARCHCLAW_AGENTS.md` 鈫?aprende el rol de orquestador de investigacion
2. OpenClaw lee `README.md` 鈫?comprende la instalacion y la estructura del pipeline
3. OpenClaw copia `config.researchclaw.example.yaml` 鈫?`config.yaml`
...
### 馃攲 Bridge de OpenClaw (Avanzado)
...
# config.arc.yaml
openclaw_bridge:
  use_cron: true              # 鈴?Ejecuciones de investigacion programadas
  use_message: true           # 馃挰 Notificaciones de progreso (Discord/Slack/Telegram)
  use_memory: true            # 馃 Persistencia de conocimiento entre sesiones
  use_sessions_spawn: true    # 馃攢 Generar sub-sesiones paralelas para etapas concurrentes
  use_web_fetch: true         # 馃寪 Busqueda web en vivo durante la revision de literatura
  use_browser: false          # 馃枼锔?Recopilacion de articulos basada en navegador
```
...
AutoResearchClaw puede usar **cualquier agente de programacion compatible con ACP** como backend LLM 鈥?sin necesidad de claves API. El agente se comunica via [acpx](https://github.com/openclaw/acpx), manteniendo una sola sesion persistente a traves de las 23 etapas del pipeline.
...
```yaml
# config.yaml 鈥?ejemplo ACP
llm:
  provider: "acp"
  acp:
    agent: "claude"   # Cualquier comando CLI de agente compatible con ACP
    cwd: "."          # Directorio de trabajo para el agente
  # No se necesita base_url ni api_key 鈥?el agente gestiona su propia autenticacion.
```

```bash
# Solo ejecuta 鈥?el agente usa sus propias credenciales
researchclaw run --config config.yaml --topic "Your research idea" --auto-approve
```

### 馃洜锔?Otras formas de ejecucion

| Metodo | Como |
|--------|------|
| **CLI independiente** | `researchclaw setup` 鈫?`researchclaw init` 鈫?`researchclaw run --topic "..." --auto-approve` |
...
## 馃敩 Pipeline: 23 etapas, 8 fases

```
Fase A: Alcance de investigacion     Fase E: Ejecucion de experimentos
  1. TOPIC_INIT                        12. EXPERIMENT_RUN
  2. PROBLEM_DECOMPOSE                 13. ITERATIVE_REFINE  鈫?auto-reparacion
...
Fase C: Sintesis de conocimiento       17. PAPER_DRAFT
  7. SYNTHESIS                         18. PEER_REVIEW        鈫?verif. evidencia
...
Fase D: Diseno experimental          Fase H: Finalizacion
 ...
 11. RESOURCE_PLANNING                 22. EXPORT_PUBLISH     鈫?LaTeX
                                       23. CITATION_VERIFY    鈫?verif. relevancia
```
...
> **Bucles de decision**: La etapa 15 puede activar REFINE (鈫?Etapa 13) o PIVOT (鈫?Etapa 8), con versionado automatico de artefactos.

<details>
<summary>馃搵 Que hace cada fase</summary>
...
| **B: Literatura** | Busqueda multi-fuente (OpenAlex 鈫?Semantic Scholar 鈫?arXiv) de articulos reales, filtrado por relevancia, extraccion de fichas de conocimiento |
...
| **D: Diseno** | Disena plan experimental, genera Python ejecutable adaptado al hardware (nivel de GPU 鈫?seleccion de paquetes), estima necesidades de recursos |
...
| **G: Redaccion** | Esquema 鈫?redaccion seccion por seccion (5,000-6,500 palabras) 鈫?revision por pares (con consistencia metodologia-evidencia) 鈫?revision con guardia de longitud |
...
```bash
# 3. Ejecuta como siempre 鈥?MetaClaw funciona de forma transparente
...
- **Sin nuevas dependencias.** MetaClaw es opcional 鈥?el pipeline base funciona sin el.
- **Los 1,823 tests existentes pasan** con el codigo de integracion presente.

---

## 鈿欙笍 Referencia de configuracion
...
experiment:
  ...
  docker:
    ...
    auto_install_deps: true        # Deteccion automatica de imports 鈫?requirements.txt
  ssh_remote:
...
- 馃敩 [AI Scientist](https://github.com/SakanaAI/AI-Scientist) (Sakana AI) 鈥?Pionero en investigacion automatizada
- 馃 [AutoResearch](https://github.com/karpathy/autoresearch) (Andrej Karpathy) 鈥?Automatizacion de investigacion de principio a fin
- 馃寪 [FARS](https://analemma.ai/blog/introducing-fars/) (Analemma) 鈥?Sistema de investigacion completamente automatizado

---

## 馃搫 Licencia

MIT 鈥?consulta [LICENSE](../LICENSE) para mas detalles.
...
<p align="center">
  <sub>Construido con 馃 por el equipo de AutoResearchClaw</sub>
</p>
