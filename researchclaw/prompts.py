...
_DEFAULT_BLOCKS: dict[str, str] = {
    ...
    "compute_budget": (
        ...
        "- MANDATORY: checkpoint partial results after every seed/condition so progress\n"
        "  is preserved if the external sandbox timeout, UI stop, or machine reboot is reached.\n"
        "  Use ExperimentHarness.log_result(...) immediately after each seed; it writes\n"
        "  partial_results.json atomically. On resume, skip completed condition/seed rows\n"
        "  from resume_seed_registry.json and continue with the next unfinished seed.\n"
        "  Do NOT stop at 80% of budget or skip later conditions solely because a soft budget guard fired.\n"
        ...
        "  # In your experiment loop, checkpoint after each seed/condition.\n"
        "  # harness.should_stop() is intentionally non-stopping; the sandbox timeout\n"
        ...
        "  harness.log_result({'condition_name': condition, 'seed': seed,\n"
        "                      'primary_metric': value})  # checkpoints partial_results.json\n"
        "  # At the end of ALL experiments:\n"
        "  harness.finalize()  # writes results.json — MUST be called\n"
        "  ```\n"
        "  The harness provides: metric validation, NaN/Inf detection,\n"
...
_DEFAULT_SUB_PROMPTS: dict[str, dict[str, Any]] = {
    ...
    "iterative_improve": {
        ...
        "user": (
            ...
            "{exp_plan_anchor}"
            "METHOD DIAGNOSIS AND ADVANTAGE GATE (CRITICAL):\n"
            "{method_diagnosis}\n"
            "- If the diagnosis says method redesign is required, DO NOT spend this "
            "iteration on logging-only or entrypoint-only edits.\n"
            "- Make changes that directly improve the target method or evaluation logic "
            "for the claimed advantage.\n"
            "- If no substantive method redesign is needed, keep code unchanged and say so "
            "explicitly in the method-change JSON block.\n\n"
            ...
            "- First include a JSON block describing the method change:\n"
            "```json\n"
            "{\n"
            "  \"method_change_summary\": {\n"
            "    \"redesign_goal\": \"one sentence\",\n"
            "    \"changed_conditions\": [\"condition_a\"],\n"
            "    \"advantage_targets\": [\"quality\", \"time\", \"vram\", \"rank\", \"trigger\"],\n"
            "    \"substantive_method_change\": true,\n"
            "    \"expected_effect\": \"what this change should improve\"\n"
            "  }\n"
            "}\n"
            "```\n"
            "- Then return the updated files using ```filename:xxx.py blocks.\n\n"
...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "experiment_design": {
        ...
        "user": (
            ...
            "(estimated from total budget / planned conditions; use the full configured budget).\n"
            ...
            "- HARD CAP: total_conditions × num_seeds × seconds_per_run MUST fit within "
            "{time_budget_sec}s. Do not add a hard 80% early-stop guard; instead checkpoint "
            "after every seed and let the sandbox timeout be authoritative.\n"
    ...
    "code_generation": {
        ...
        "user": (
            ...
            "- If the experiment plan depends on downloadable datasets or pretrained assets, "
            "put that logic in setup.py only. main.py must assume assets are already prepared.\n"
            "- Do NOT perform runtime dataset downloads inside main.py / data.py.\n"
            "- If using PyTorch mixed precision, LoRA deltas, or adapter updates, explicitly align "
            "device and dtype before addition/matmul (e.g. delta = delta.to(x.device, dtype=x.dtype)).\n"
            "- Avoid in-place mutation on tensors that participate in autograd across steps.\n"
            ...
            "MULTI-GPU SEMANTICS:\n"
            "- The runtime may expose up to three GPUs. Your code must remain correct on one, two, or three GPUs.\n"
            "- If using multiple GPUs, explicitly document the strategy and ensure metrics are aggregated consistently.\n"
            "- If your experiment does NOT support multi-GPU safely, keep a single-GPU code path and make that explicit.\n"
            "- Print the actual device plan at startup: `DEVICE_PLAN: single_gpu|multi_gpu|cpu` and `VISIBLE_GPUS: ...`.\n\n"
